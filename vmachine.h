...
#define VMACHINE_INSTR_SET \
	X(halt) X(nop) \
    X(immA) X(immB) X(incA) X(incB) X(decA) X(decB) X(zero) \
    X(add) X(sub) X(mul) X(divi) X(mod) \
	X(_and) X(_or) X(_xor) X(sll) X(srl) X(sra) \
	X(slt) X(sltu) X(cmp) \
...
#define INSTR_PARAMS    union VMachinePtr pc, union VMachineVal rA, union VMachineVal rB, uintptr_t sp, uintptr_t fp, uintptr_t lr, struct VMachineState *state
#define INSTR_ARGS      pc, rA, rB, sp, fp, lr, state
